...
def create_figure1():
    ...
    # 1. User Input (Top)
    create_box(ax, 5, 14.5, 3, 0.6, 'User Query\n(Python API or MCP Client)',
               color_input, fontsize=10, fontweight='bold')

    # MCP Resources sidebar (right side - browsable resources exposed by MCP server)
    create_box(ax, 8.2, 14.5, 1.8, 1.0, 'MCP Resources\n• cell-types\n• network/{ct}\n• gene/{symbol}',
               color_data, fontsize=7)
    create_arrow(ax, 6.5, 14.5, 7.25, 14.5, style='dashed', color='gray', linewidth=1.5)
    ...
    # Cross-cell comparison (conditional, left side - hub/master regulators or >15 regulators)
    create_box(ax, 1.5, 7.0, 2.0, 0.5, 'Cross-Cell\nComparison',
               color_core, fontsize=8, fontweight='bold')
    ax.text(1.5, 6.65, '(hub/master regulators)', fontsize=6.5, ha='center',
            va='center', style='italic', color='#555555')
    create_arrow(ax, 3.95, 7.1, 2.55, 7.05, style='dashed', color='red', linewidth=2)
    create_arrow(ax, 2.55, 6.75, 3.95, 6.6, style='dashed', color='red', linewidth=2)

    # 7. Parallel Domain Analysis (4 agents)
    # Header box
    create_box(ax, 5, 6.3, 4.5, 0.5, 'Parallel Domain Analysis (Rule-Based + Optional LLM)',
    ...
    # Copy to project root for paper.md reference
    root_dest = os.path.join(project_root, 'figure1_architecture.png')
    shutil.copy(os.path.join(output_dir, 'figure1_architecture.png'), root_dest)
    print(f"[OK] Figure 1 copied to root: {root_dest}")
...
def create_figure2():
    ...
    steps = [
        ("STRING Database\nLookup", "5-10 min", 11),
        ("Reactome/Enrichr\nPathway Enrichment", "10-15 min", 8.3),
        ("PubMed Literature\nCuration", "60-120 min", 5.6),
        ("Manual Integration\n& Synthesis", "30-60 min", 2.9)
    ]

    for i, (step_text, time_text, y_pos) in enumerate(steps):
        # Step box (gray)
        box = FancyBboxPatch((0.5, y_pos-0.5), 7.5, 1.0,
        ...
        # Arrow to next step
        if i < len(steps) - 1:
            arrow = FancyArrowPatch((4.25, y_pos-0.6), (4.25, steps[i+1][2]+0.6),
    ...
    parallel_box = FancyBboxPatch((1, 6.8), 8, 2.2,
                                  boxstyle="round,pad=0.1",
    ...
    for agent_text, x, y in agents:
        mini_box = FancyBboxPatch((x-0.7, y-0.4), 1.4, 0.8,
    ...
    # Output box
    output_box = FancyBboxPatch((2, 4.3), 6, 0.9,
                               boxstyle="round,pad=0.1",
    ...
    manual_times = [2.5*3600, 2.5*3600, 12.5*3600, 12.5*3600]  # Convert hours to seconds
    regnet_times = [
        timing_data['rule_based']['single_comprehensive_sec'],
        timing_data['llm_powered']['single_comprehensive_sec'],
        timing_data['rule_based']['multi_5genes_sec'],
        timing_data['llm_powered']['multi_5genes_sec']
    ]

    # Calculate speedups
    speedups = [manual/regnet for manual, regnet in zip(manual_times, regnet_times)]
