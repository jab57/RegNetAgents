...
Therapeutic prioritization uses NetworkX's deterministic PageRank and degree-based centrality metrics to rank upstream regulators by influence within the ARACNe-derived subnetwork. The four domain agents each produce an independent evidence assessment (high/moderate/low) for a specific biological dimension: oncogenic potential (cancer agent), druggability (drug discovery agent), clinical actionability (clinical relevance agent), and network centrality (systems biology agent). The rule-based layer—active by default (`USE_LLM_AGENTS=false`)—derives these assessments from network topology using domain-specific, biologically motivated thresholds that reflect established distinctions in regulatory influence: for example, the cancer agent requires >20 downstream targets for high oncogenic potential, while the systems biology agent uses a lower bar of >10 targets combined with regulatory role and PageRank >0.01. **High** is assigned when a gene satisfies two or more domain-specific criteria; **moderate** when one criterion is met; **low** when none apply. The multi-criteria requirement for **high** reduces false positives by requiring independent convergent signals. These thresholds serve as biologically informed defaults and can be adjusted for specific network types or research contexts; full threshold specifications are available in the source code. A cross-domain contradiction checker automatically flags logical inconsistencies across the four domain assessments, including:
