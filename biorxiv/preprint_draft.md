...
**Performance:** LLM mode adds ~5 seconds per gene (4 agents in parallel, ~1.25s each) compared to rule-based mode. For 5-gene analysis: ~6 seconds (rule-based) vs ~20 seconds (LLM). Both modes remain orders of magnitude faster than manual literature review.
